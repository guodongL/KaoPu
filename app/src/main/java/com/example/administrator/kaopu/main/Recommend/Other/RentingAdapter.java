...
public class RentingAdapter extends BaseAdapter{
    ...
    private List<RentingBean> list;
    public RentingAdapter(Context mContext,List<RentingBean> list){
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (size==null){
            mHolder.text_size.setText(list.get(position).getFamilyType());
        }else {
    ...
    public void reloadGridView(List<RentingBean> _list, boolean isClear) {
        if (isClear) {
            list.clear();
        }
        list.addAll(_list);
